...
(async () => {
  ...
  console.log(
    `  ways: ${intLines.length} / pts ${totalBefore} → ${totalAfter} (${(100 - (100 * totalAfter) / Math.max(1, totalBefore)).toFixed(1)}% 削減)`
  );
  if (intLines.length === 0) {
    console.error('❌ no hiking trails');
    process.exit(1);
  }
  ...
  const data = {
    ...
    lines: linesB64,
    source:
      'OpenStreetMap (ODbL)・sac_scale または foot=designated 付き path / footway / bridleway',
  };
