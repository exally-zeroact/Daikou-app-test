...
// ★印は10個では足りない（2026-08-22 実測）★
//   わざと関数を書き換えても ★頭から10個の印に当たらなければ 緑のまま★だった。
//   ＝「真ん中しか見ていない」型。実際に 10/25/40/60/100 で回して
//     ★100個でも 4本とも全部一致★（配る時に組み直されても 印は消えない）と確かめたので 100 にする。
export function fingerprint(src, max = 100) {
  const out = [];
  const seen = new Set();
  const push = (s) => {
