...
if (!bbox) {
  console.error(`Unknown prefecture: ${PREF}`);
  process.exit(1);
}
...
async function fetchKsjPolygon(label, urls, attrMap, outName) {
  ...
  report.push({
    source: label,
    status: 'failed',
    reason: lastErr ? lastErr.message : 'all URLs failed',
    tried: urls,
  });
}
...
(async () => {
  log(
    `[fetch-real-data] prefecture=${PREF} bbox=[${bbox.south},${bbox.west} - ${bbox.north},${bbox.east}]`
  );
  log(`  out: ${OUT_DIR}`);
  ...
  report.push({
    ...
    status: 'skipped',
    reason:
      '公開ポリゴンAPI無し（地理院の液状化マップはラスタ画像タイルのみ・KSJに該当データセット無し）。J-SHIS/防災科研は確率値の格子データのため整合せず。県別データの取得は自治体公開Shapefileの個別ダウンロードが必要。',
  });
  ...
  try {
    ...
    for (const f of fc.features)
      byCat[f.properties.category] = (byCat[f.properties.category] || 0) + 1;
    report.push({
      source: 'POI',
      status: 'ok',
      file: 'poi.geojson',
    ...
    log(
      `  ✅ POI: ${fc.features.length} 件 / ${(size / 1024).toFixed(1)} KB / カテゴリ ${Object.keys(byCat).length} 種`
    );
  } catch (e) {
  ...
  try {
    ...
    log(
      `  ${fc.features.length > 0 ? '✅' : '⚠️ '} スクールゾーン: ${fc.features.length} 件 / ${(size / 1024).toFixed(1)} KB`
    );
    if (fc.features.length === 0) {
      report[report.length - 1].note =
        'OSM の hazard=school_zone タグはまだ整備が薄く 0 件になりがち。県オープンデータからの取り込みが本筋。';
    }
  } catch (e) {
  ...
  report.push({
    ...
    status: 'skipped',
    reason:
      'KSJ N04/N06 の県別URLパターンが見つからず、自動取得不可。各県の防災基本計画に紐づく本データは、愛媛県オープンデータカタログ（https://www.pref.ehime.jp/h12200/opendata/）または国土数値情報「緊急輸送道路」(https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N04.html) からのShape/CSV手動DLが必要。',
      ...
      r.status === 'ok'
        ...
        : r.status.startsWith('ok-')
          ...
          : r.status === 'skipped'
            ...
            : '❌';
    log(
      `  ${tag} ${r.source.padEnd(12)} ${r.status.padEnd(20)} ${r.count != null ? `count=${r.count}` : ''} ${r.bytes != null ? `size=${(r.bytes / 1024).toFixed(1)}KB` : ''}`
    );
  }
