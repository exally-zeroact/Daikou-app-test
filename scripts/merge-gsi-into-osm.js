  ...
  for(const f of gsiFiles){
    ...
    if(fc && Array.isArray(fc.features)){
      for(const feat of fc.features){
        if(feat.geometry && feat.geometry.type === 'LineString' && feat.geometry.coordinates.length >= 2){
    ...
    for(const [lng, lat] of osmCoords){
      ...
      for(let dy = -1; dy <= 1; dy++){
        for(let dx = -1; dx <= 1; dx++){
          ...
          const nk = (Math.floor(lat / GRID_DEG) + dy) + '_' +
                     (Math.floor(lng / GRID_DEG) + dx);
          const nArr = gsiGrid.get(nk);
          if(nArr) for(const i of nArr) candIdx.add(i);
        }
      }
    }
    let bestIdx = -1, bestDist = PROXIMITY_M;
...
function main(){
  ...
  console.error('[merge] coverage rate: ' +
