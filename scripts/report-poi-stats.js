...
console.log(
  `平均: ${Math.round(grandTotal / files.length).toLocaleString()} POI / 県, ${(grandSize / files.length / 1024).toFixed(1)} KB / 県`
);
...
for (const r of Object.keys(REGIONS)) {
  ...
  console.log(
    `${r.padEnd(10)} ${prefs.length}県 / ${t.toLocaleString().padStart(10)} POI / ${(s / 1024 / 1024).toFixed(2)} MB`
  );
}
...
for (const [pref, st] of sorted) {
  ...
  console.log(
    `  ${pref.padEnd(12)} ${st.region.padEnd(10)} ${st.total.toLocaleString().padStart(8)} POI  ${sizeKB.padStart(8)} KB  zero-cats=${st.zeroCats.length}`
  );
}

console.log('\n===== 0件カテゴリのある県 =====');
const zeroCatPrefs = Object.entries(stats)
  .filter(([_, s]) => s.zeroCats.length > 0)
  .sort((a, b) => b[1].zeroCats.length - a[1].zeroCats.length);
for (const [pref, st] of zeroCatPrefs) {
  console.log(
    `  ${pref.padEnd(12)} 0件カテゴリ数=${st.zeroCats.length}: ${st.zeroCats.join(', ')}`
  );
}
...
for (const [id, n] of catSorted) {
  console.log(
    `  [${id.padStart(3)}] ${CATEGORIES[id].padEnd(20)} ${n.toLocaleString().padStart(8)}`
  );
}
