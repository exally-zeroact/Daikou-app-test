...
(async () => {
  ...
  for (const region of Object.keys(REGION_BBOX)) {
    ...
    console.log(`▼ ${region} bbox=[${bbox.join(',')}] cells=${cells.length} types=${plan.types.length}グループ`);

    let regionWays = 0, regionDup = 0;
    for (let ci = 0; ci < cells.length; ci++) {
      ...
      for (let ti = 0; ti < plan.types.length; ti++) {
        ...
        if (fs.existsSync(cache) && (Date.now() - fs.statSync(cache).mtimeMs) < 7 * 86400000) {
          console.log(`  cell ${ci+1}/${cells.length} types=[${typeGroup.join(',')}] cache: ${path.basename(cache)}`);
          json = JSON.parse(fs.readFileSync(cache, 'utf8'));
        } else {
          console.log(`  cell ${ci+1}/${cells.length} types=[${typeGroup.join(',')}] bbox=[${cell.map(n=>n.toFixed(2)).join(',')}]`);
          json = await fetchOverpass(buildQuery(cell, typeGroup), 600000);
          fs.writeFileSync(cache, JSON.stringify(json));
        }
        for (const el of (json.elements || [])) {
          if (el.type !== 'way' || !el.geometry) continue;
          if (seenIds.has(el.id)) { regionDup++; continue; }
          seenIds.add(el.id);
          const tags = el.tags || {};
  ...
  if (intLines.length === 0) { console.error('❌ no waterways'); process.exit(1); }
