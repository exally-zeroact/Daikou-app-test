#!/usr/bin/env node
/**
 * build-bundle-railways-jp.js
 *
 * 全国の鉄道路線 (railway=rail|light_rail|subway|tram|monorail|narrow_gauge|funicular)
 * を Overpass から取得し data/railways-jp.js に出力。
 *
 * 構造 (coastline 同型 + typeCode):
 *   types[i] : 0=rail 1=light_rail 2=subway 3=tram 4=monorail 5=narrow_gauge 6=funicular
...
const OUT = path.join(PROJECT_ROOT, 'data', 'railways-jp.js');
const CACHE = path.join(TMP, 'railways-overpass.json');
...
const QUERY =
  ...
  '(' +
    `way["railway"~"^(${RAILWAY_TYPES})$"](area.jp);` +
  ');' +
  'out tags geom;';

const TYPE_CODES = {
  rail: 0, light_rail: 1, subway: 2, tram: 3,
  monorail: 4, narrow_gauge: 5, funicular: 6,
};
...
(async () => {
  ...
  const TOL_DEG = 0.0001; // ≈10m (鉄道は曲線少なめ・大きめ簡略 OK)
  ...
  for (const el of (json.elements || [])) {
    ...
    seenIds.add(el.id);
    const tags = el.tags || {};
    const t = TYPE_CODES[tags.railway];
    ...
    counts[tags.railway] = (counts[tags.railway] || 0) + 1;
  }

  console.log(`  ways: ${intLines.length}`);
  console.log(`  内訳:`, counts);
  console.log(`  pts ${totalBefore} → ${totalAfter} (${(100-100*totalAfter/Math.max(1,totalBefore)).toFixed(1)}% 削減)`);
  if (intLines.length === 0) { console.error('❌ no railways'); process.exit(1); }
  ...
  const data = {
    ...
    grid,
    types: typesB64,
    typeLegend: { 0: 'rail', 1: 'light_rail', 2: 'subway', 3: 'tram', 4: 'monorail', 5: 'narrow_gauge', 6: 'funicular' },
    lines: linesB64,
    source: `OpenStreetMap (ODbL) railway=${RAILWAY_TYPES}`,
  };

  const size = u.writeBundleJs(OUT, 'RAILWAYS_JP', data, [
    `// 出典: OpenStreetMap (ODbL)`,
    `// 全国 ${intLines.length} ways (鉄道線路)`,
    `// 内訳: ${Object.entries(counts).map(([k,v]) => `${k}:${v}`).join(' / ')}`,
  ]);
