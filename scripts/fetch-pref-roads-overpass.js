...
async function main() {
  ...
  if (!pref) { console.error('Usage: fetch-pref-roads-overpass.js <pref>'); process.exit(1); }
  const jaName = PREF_NAMES_JA[pref];
  if (!jaName) { console.error(`unknown pref: ${pref}`); process.exit(1); }
  ...
  console.log(`  raw タグ充足: oneway ${pct(stats.oneway)} / incline ${pct(stats.incline)} / lanes ${pct(stats.lanes)} / width ${pct(stats.width)} / layer ${pct(stats.layer)}`);
}
