...
if (!PREF) {
  console.error('Usage: fetch-poi-pbf-local.js <pref>');
  process.exit(1);
}
...
if (!info) {
  console.error(`Prefecture not in meta.json: ${PREF}`);
  process.exit(1);
}
...
async function downloadPBF() {
  ...
  if (fs.existsSync(PBF)) {
    ...
    if (age < 24 * 3600 * 1000) {
      console.log(
        `  [1/3] DL skip (cache <24h): ${PBF} (${(fs.statSync(PBF).size / 1024 / 1024).toFixed(1)} MB)`
      );
      return;
    }
  }
...
async function streamFilter() {
  ...
  console.log(
    `       parsed nodes=${nodeCount.toLocaleString()}  poi-tagged=${matchedTagCount.toLocaleString()}  in-bbox=${inBboxCount.toLocaleString()}`
  );
  return features;
}
...
(async () => {
  ...
  console.log(`\n=========================================`);
  console.log(
    `✅ ${PREF}: ${features.length.toLocaleString()} 件 / ${(size / 1024).toFixed(1)} KB → ${outPath}`
  );
  console.log(`=========================================`);
  console.log(`カテゴリ別:`);
