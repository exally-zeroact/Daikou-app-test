...
const FB = (() => {
  ...
  function _openRetryDb(){
    return new Promise((resolve, reject) => {
      if(_retryDb){ resolve(_retryDb); return; }
      try {
        ...
        req.onsuccess = function(){ _retryDb = req.result; resolve(_retryDb); };
  ...
  function _enqueueRetry(type, payload){
    _openRetryDb().then(db => {
      ...
      } catch(e){ console.error('[FB.retry] enqueue error:', e); }
    }).catch(e => console.error('[FB.retry] db open error:', e));
  }

  function _retryGetAll(){
    return _openRetryDb().then(db => new Promise((resolve, reject) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        if(record.type === 'startSession'){
          db.ref('sessions_log/' + record.payload.sessionId).set(record.payload.data)
            .then(() => resolve()).catch(reject);
        } else if(record.type === 'endSession'){
          db.ref('sessions_log/' + record.payload.sessionId).update(record.payload.data)
            .then(() => resolve()).catch(reject);
        } else if(record.type === 'saveFareConfig'){
          db.ref('fare_config/default').set(record.payload)
            .then(() => resolve()).catch(reject);
        } else {
  ...
  window.addEventListener('online',  () => { isOnline = true; flushQueue(); _flushRetryQueue(); });
  ...
  function startSession(fareConfigSnapshot){
    ...
    db.ref('sessions_log/' + sessionId).set(sessionData).catch(e => {
  ...
  function endSession(finalState){
    ...
    db.ref('sessions_log/' + _sid).update(sessionUpdate).catch(e => {
  ...
  function saveFareConfig(config){
    const db = getDb(); if(!db) return;
    // ★設計変更宣言 (2026-05-12): 送信失敗時 retry queue 登録 (成功パス無変更)
    db.ref('fare_config/default').set(config).catch(e => {
