...
test('★★④ 入力タブが 在り、開かずに 打てる★★', async () => {
  ...
  // ★★見る 範囲を 広げた★★ 2026-09-06（司さん「入力タブは」）
  //   日の 帯（PayPay）を 足して 中身が 長く なり、3000字では
  //   ★bindYen() が 範囲の 外に 出て 偽の 赤★に なった（実測）。
  const naka = SRC.slice(i, i + 5000);
...
test('★★⑤ 会社が 足した 名前が 入力タブに 出る★★', async ({ page }) => {
  ...
  // ★★2026-09-06 に 形を 変えた（司さん「入力タブは」）★★
  //   ★前★ 1行＝日・車・実費…（日が 何度も 出る）
  //   ★今★ ★日の 帯★（そこに PayPay）＋ その下に 車ごとの 行
  //   ⇒ 見出しから ★日が 消えた★ … 車 ＋ 使う 名前 4つ ＝ ★5★
  //   ⇒ 欄は 実費 4つ ＋ ★PayPay 1つ★ ＝ ★5★
