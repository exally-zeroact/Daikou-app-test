...
function makeGpsTrace(road, gpsLengthM, gpsStepM, noiseM, t0, speedKmh, seed){
  ...
  for(let m = 0; m <= gpsLengthM; m += gpsStepM){
    ...
    gps.push({
      ...
      accuracy: 5, speedKmh: speedKmh, headingDeg: 0,
    });
  }
  return gps;
}
...
function makeStraight1km(){
  const road = buildRoad(34.0658, 132.9970, [{ bearing: 0, length: 1100 }]);
  const gps = makeGpsTrace(road, 1000, 10, 3, 1714000000000, 36, 42);
  const expected = 1000;
  writeJsonl('synthetic-straight-1km.jsonl', {
...
function makeCurve15km(){
  ...
  writeJsonl('synthetic-curve-1.5km.jsonl', {
...
function makeOverpass(){
  const ground = buildRoad(33.8800, 132.7700, [{ bearing: 90, length: 800 }]);
  ...
  writeJsonl('synthetic-overpass-700m.jsonl', {
