...
async function callOsrmMatch(coords){
  ...
  const url = ENDPOINT + '/match/v1/' + PROFILE + '/' + coordStr +
...
async function main(){
  const files = fs.readdirSync(FIXTURES_DIR).filter(f => f.endsWith('.jsonl')).sort();
  const results = [];
  let okCount = 0, failCount = 0, skipCount = 0;
  let sumDaikou = 0, sumOsrm = 0;
  ...
  for(const f of files){
    ...
    try {
      ...
      results.push({
        ...
        der_vs_expected: +(Math.abs(dOsrm - meta.expected_distance_m) / meta.expected_distance_m).toFixed(5),
      });
      sumOsrm += dOsrm;
      sumDaikou += meta.expected_distance_m;
      okCount++;
      console.log('  ' + meta.name + ': osrm=' + dOsrm.toFixed(1) + 'm expected=' + meta.expected_distance_m + 'm');
    } catch(e){
  ...
  console.log('[tier1] updated latest.json (ok=' + okCount + ' fail=' + failCount + ' skip=' + skipCount + ')');
}

main().catch(e => { console.error('[tier1] fatal:', e); process.exit(0); });  // 失敗しても CI fail にしない
