...
function replayFixture(file){
  ...
  let totalCount = 0, snapCount = 0, skipCount = 0, mmDistance = 0;
  let layerHits = 0, layerChecks = 0;
  let prevSnap = null;
  const issues = [];
...
function main(){
  ...
  for(const f of files){
    ...
    console.log('  ' + r.name + ': mm=' + r.mm_distance_m + 'm exp=' + r.expected_distance_m +
                'm DER=' + (r.distance_error_ratio * 100).toFixed(2) + '%' +
                ' snap=' + (r.snap_rate * 100).toFixed(1) + '%');
    results.push(r);
  }
  ...
  const summary = {
    ...
    distance_error_ratio_overall: sumExpected > 0 ? +(Math.abs(sumMM - sumExpected) / sumExpected).toFixed(5) : 0,
  ...
  const out = {
    ...
    tiers: {
      ...
      tier2_self: { status: 'ok', note: 'replay-mm.js (this run)', der: summary.distance_error_ratio_overall },
      tier4_google: { status: 'not_run', note: 'monthly limited / set GOOGLE_DIRECTIONS_API_KEY secret' },
    },
    issues: allIssues,
  };
  ...
  console.log('[replay] overall snap_rate=' + (summary.snap_rate_overall * 100).toFixed(2) +
              '% DER=' + (summary.distance_error_ratio_overall * 100).toFixed(3) + '%');
}
