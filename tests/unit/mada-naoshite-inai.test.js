...
const SOFT = {
  ...
  'fare-history': {
    なぜsoft:
      '★赤で正しい★（★試験を 先に 書いた・本体 js/fare-history.js は まだ 無い★）。' +
      '今 料金を保存する所は ★上書き1件だけ★（js/firebase.js:346）＝★間違えても 戻せない★。' +
      'C1 で 社長が 変えられるようにする＝★戻せない物を 触らせる★ので ★戻す道を 先に 作る★。',
    いつ外すか:
      '★js/fare-history.js を 作って 9本 全部 緑に なった日★。' +
      '★線を 緩めて 緑にするのは 不可★（それをやると 何も見ていない緑）。',
    実測: '2026-08-29 … ★9本 見て 9本とも 赤★（本体が 無い＝正しい赤）／1秒未満',
    お金への影響:
      '★無し（今は）★ … 記録と 戻す道の 話で、★金額の計算には 触りません★。' +
      '★過去の請求の写し（fare_config_snapshot）も 1件も 触りません★。',
  },
  'gnss-degraded': {
    なぜsoft:
...
describe('★非ブロック指定（soft）の棚卸し★', () => {
  ...
  it('★soft の 中身の内訳が 変わっていない★（赤5・未測定0）', () => {
