...
describe('★配る物に 作った時刻を 焼き付けない★', () => {
  ...
  // ★★2026-08-29: ここが たまに 赤でした。原因を 捕まえました★★
  //   ★数え間違いでは ありません★（前に そう書いたのは 私の 読み違い）。
  //   記録係(tests/tools/failure-recorder.js)が 残した 実物の文言:
  //     「Test timed out in 5000ms.」
  //   ＝★5秒の 時間切れ★でした。
  //
  //   ★「重いから」で 片づけません。測りました★（2026-08-29 実測）
  //     道路データ 47本＝合わせて 204MB。頭 400バイトだけ 読むのに かかる時間:
  //       1回目(冷えている) 72ms ／ 2回目 11ms ／ 3回目 10ms
  //     ＝★この試験 自体は 5秒の 70分の1★。重いのではありません。
  //   ⇒ 赤に なったのは ★他の試験と 重なって ディスクが 詰まった時の 1回目★。
  //     （18回 回して 2回。どちらも npm ci の直後／file を 書き直した直後）
  //   ⇒ ★中身は 正しいのに 時間切れで 赤★＝一番いけない「嘘の赤」。
  //     時間の余裕を 30秒に します（実測の 400倍）。★判定は 1文字も 緩めていません★。
  it('★今 repo に在る道路データにも 時刻が入っていない★（入っていたら 刷り直しが要る）', () => {
    ...
    // ★★2026-08-30: 予告どおり ★0本★ に なりました★★
    //   前は「47本 全部に 残っている（2026-08-28 実測）／次に osm-update（毎週）が
    //   回って 刷り直されると 0本 に減る ⇒ 減ったら ここを 0 に直す」と 書いてありました。
    //   ★実際に 起きました★: origin/main 69670104 で 47本 全部から
    //   「// Generated: …」が 消えています（私の手元は 古い版だったので 気づかず、
    //   ★CI が main と 合わせた版で 測って 教えてくれました★）。
    //   ⇒ ★見張りが 予告どおり 働いた例★。★0本に 直します★。
    //   ★増えたら 赤★（別の作り方で 時刻が また 入った、という合図）
    expect(
      nokotteru.length,
      '★道路データの「Generated:」の本数が 0本から 変わりました★\n' +
        '  ・増えた … ★別の作り方で 時刻が また入りました★（刷り直しが 要ります）\n' +
        '  ・減った … ★もう 0本です。これ以上 減りません★'
    ).toBe(0);
  }, 30000); // ★30秒★（実測 72ms・詰まった時だけ 効く保険）
});
...
const SOFT = {
  'coast-cap': {
    なぜsoft:
      '★赤で正しい★（★下（取りこぼし）だけ 赤・上（過大）は 0件★）。' +
      '2026-08-30・上と下を ★両方★ 見る門。' +
      '合成点が 10秒以下の 間隔で 来ると cap(coastHoleMaxSec=25秒)まで ★500m だけ 乗って 止まる★。',
    いつ外すか:
      '★司さんが 直し方を 決めた日★。★2026-08-30 に 直しを 1回 試して 戻しました★' +
      '（実機 0610-Android(OBD) で distance_m 7,701m > 天井 4,948m＝★過大★に なった為）。',
    実測:
      '2026-08-30 手元 … 合成点の間隔・凍結／本当に走った／出た: ' +
      '10秒 20秒 600.0m→374.5m ／ 10秒 40秒 1000.0m→674.5m ／ 10秒 60秒 1400.0m→674.5m ／ ' +
      '5秒 40秒 1000.0m→674.5m ／ 5秒 100秒 2200.0m→674.5m ／ 10秒 40+40秒 2000.0m→1349.0m。' +
      '★おかしい所 6件は 全部 下（取りこぼし）・上（過大）は 0件★。' +
      '500m ＝ 25秒×20m/s と ぴったり 一致。',
    お金への影響:
      '★有り（少なく請求する側）★ … 会社が 損をする向き。客に 多く請求しては いません。' +
      '★直し方は まだ 決まっていません★（試した直しが 実機で 過大に なり 戻した）。',
  },
  'obd-mienai': {
    なぜsoft:
      '★赤で正しい★（2026-08-30 に 作った ★過少（落ちる側）を 見る 2本目★）。' +
      '1本目(bg-freeze-nagaana)は ★位置が 進まない★形／この門は ★点が 1つも 来ない★形で ★別物★。' +
      'きっかけは 司さんの申告「300円ほど 少なく出た」＝420m刻み×3段＝約1,260m。',
    いつ外すか:
      '★司さんが 直し方を 決めた日★。★直すと 客への 請求が 増えます★（会社の損は 減る）ので、' +
      '10秒を 何秒にする／OBD が 落ちた時に GPS の直線へ 逃がすか は ★私が 決める話では ありません★。',
    実測:
      '2026-08-30 手元 … 20km・72km/h・OBD接続で 点が 1つも 来ない時間を 変える。' +
      '11秒→221.5m 消える／20秒→402.8m／30秒→604.2m／★60秒→1,208.3m（＝ちょうど300円）★／120秒→2,416.7m。' +
      '★同じ60秒でも OBD有り 1,208m 消える vs OBD無し 17m しか 消えない★' +
      '＝★より 正確な機械を 繋いだ方が 距離が 減る★（OBDの枝が 先に 効き dt>obdMaxDtS で 0m、' +
      'GPS の「前後を結ぶ直線」に 行き着く前に 終わる）。' +
      '★落とした事を 数えている所は 0件★（stats に obdSkipped の類が 無い）。' +
      '★門が 本物である事も 実測★: obdMaxDtS の上限を 外すと ★緑0件★・戻すと ★赤10件★。',
    お金への影響:
      '★有り（少なく請求する側）★ … 会社が 損をする向き。客に 多く請求しては いません。' +
      '★ただし 実際の走行で 再現したとは 言っていません★（trace が 2026-08-13 以降 0本）。' +
      '司さんの「画面ロックしていない」とは ★矛盾しません★（他アプリ・着信・電池節約でも 点は 止まる）。',
  },
  'bg-freeze-nagaana': {
    なぜsoft:
      '★赤で正しい★（2026-08-30 に 作った ★過少（落ちる側）を 見る 1本目★）。' +
      '他の門は 全部「多すぎたら 赤」で、★少なすぎるのを 見る門が 1本も 無かった★。' +
      'きっかけは 従業員の「こないだ いつもより 距離も 金額も 少なく出た」。',
    いつ外すか:
      '★司さんが 直し方を 決めた日★。★直すと 客への 請求が 増えます★（会社の損は 減る）ので、' +
      '10秒を 何秒にする／600m の上限を どうするかは ★私が 決める話では ありません★（指示役 2026-08-30 の裁定）。',
    実測:
      '2026-08-30 手元 … 20m/s(72km/h) の 合成した 画面ロックで ' +
      '凍結 9秒 → 184.50m（理想180m・乗る）／★凍結 11秒 → 4.50m（理想220m）★／' +
      '30秒 → 4.50m（理想600m）／60秒 → 4.50m（理想1200m）／300秒 → 4.50m（理想6000m）。' +
      '＝★10秒を 1秒 またぐと 180m → 0m★。原因は pipeline-distance.js の ★obdMaxDtS: 10★' +
      '（10→600 で 60秒の穴が 4.5m → 504.5m に 戻る／戻して sha 一致）。' +
      '＋ gps.js COAST_DECAY_PER_S=0.92 で 72km/h→9km/h まで ★約25秒★＝落ちる道が 二重。' +
      '★門が 本物である事も 実測★: 上限4つを わざと 広げると ★緑0件★・戻すと ★赤5件★。',
    お金への影響:
      '★有り（少なく請求する側）★ … 会社が 損をする向き。客に 多く請求しては いません。' +
      'ただし ★実際の走行で 再現できたとは 言っていません★（trace が 2026-08-13 以降 0本）。' +
      '棚から 拾えた 当たり候補は 1本（08-28 今治市末広町→西条市高田 15.95km／同じ組の もう1本は 22.78km）' +
      'だが ★時間も 短く 速度は 上がっている＝別の道の 可能性★があり、★原因とは 呼べません★。',
  },
  'device-spread': {
    なぜsoft:
