...
describe('★時計を 止めるなら 時間帯も 止める★', () => {
  ...
  // ★★2026-09-09 追記＝同じ 穴に 2回 落ちた★★
  //   ①ryokin-ima-kami … 深夜割増（1,300 が 1,560）
  //   ②nyuryoku-denshi … ★日付の 境目が 9時間 ずれ★、試験の 作り物が
  //      別の 日の データを 返して いた（「記録が ありません」が 出ない）
  //   ②は 時計を 止めていない ので 上の ②③では 捕まりません。
  //   ⇒ ★1本ずつ 貼る のを やめ、★全体の 決まり★に した。
  //
  //   ★実測（2026-09-09）★
  //     UTC で 181本 走らせる … ★この 2本だけ 赤★
  //     日本で 181本 走らせる … ★全部 緑★
  it('★★④ 全体の 決まりで 時間帯を 日本に 固定している★★', () => {
    const cfg = fs.readFileSync(path.join(__dirname, '..', '..', 'playwright.config.js'), 'utf8');
    expect(
      cfg.indexOf("timezoneId: 'Asia/Tokyo'"),
      '★全体の 決まりに 時間帯が ありません★' +
        '（CI は UTC で 走るので ★深夜割増★と ★日付の 境目★が ずれます）'
    ).toBeGreaterThan(-1);
  });
});
